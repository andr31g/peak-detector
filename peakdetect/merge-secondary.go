...
func mergeSecondary[T Number](left, right SecondaryPeaks[T]) SecondaryPeaks[T] {
	...
	if left.isLastSamplePeak() {
		if right.isFirstSamplePeak() {
			if leftSample == rightSample {
				...
				// up the offsets.
				return mergeSamples0[T](left, right)
			} else {
				...
				if leftSample > rightSample {
					...
					return mergeSamples0[T](left, removeContiguousPeaksFromRightSide0[T](right, rightSample))
				} else {
					...
					return mergeSamples0[T](removeContiguousPeaksFromLeftSide0[T](left, leftSample), right)
				}
			}
		} else {
			...
			if leftSample == rightSample {
				if len(right.peaks) == 0 {
					return mergeSamples0[T](left, addContiguousPeaksToRightSide0[T](right))
				} else {
					return mergeSamples0[T](removeContiguousPeaksFromLeftSide0[T](left, leftSample), right)
				}
			} else {
				...
				if leftSample > rightSample {
					...
					return mergeSamples0[T](removeContiguousPeaksFromLeftSide0[T](left, leftSample),
						addContiguousPeaksToRightSide0[T](right))
				}
			}
		}
	} else {
		if !right.isFirstSamplePeak() {
			...
			if leftSample == rightSample {
				...
				if leftSample > rightSample {
					...
					return mergeSamples0[T](addContiguousPeaksToLeftSide0[T](left), right)
				} else {
					// In this case, the last sample of the 'left' peaks cluster
			...
			if leftSample == rightSample {
				...
				if leftSample > rightSample {
					...
					// and are equal to it.
					return mergeSamples0[T](addContiguousPeaksToLeftSide0[T](left), removeContiguousPeaksFromRightSide0[T](right, rightSample))
				} else {
...
func mergeSamples0[T Number](left, right SecondaryPeaks[T]) SecondaryPeaks[T] {
	...
	primaryPeaks := append(left.primaryPeaks, right.primaryPeaks...)
	originalPeaks := append(left.originalPeaks, right.originalPeaks...)
	return CreateSecondaryPeaksWith[T](CreatePeaksWith[T](samples, peaks), primaryPeaks, originalPeaks)
}
...
func addContiguousPeaksToLeftSide0[T Number](left SecondaryPeaks[T]) SecondaryPeaks[T] {
	if len(left.peaks) == 0 {
		...
		// Pass the 'left.originalPeaks' as the new 'primaryPeaks', because we just
		// synthetically created all the peak indices, and now we need to set the
		// 'primaryPeaks' to the original peak indices, i.e., the peak indices in the
		// primary array
		return CreateSecondaryPeaksWith[T](CreatePeaksWith[T](left.samples, left.createAllPeaks()), left.originalPeaks, left.originalPeaks)
	} else {
		return left
	}
}
...
func removeContiguousPeaksFromLeftSide0[T Number](left SecondaryPeaks[T], sampleValue T) SecondaryPeaks[T] {
	...
	peaks := CreatePeaksWith[T](left.samples, left.peaks[:len(left.peaks)-contiguousEqualSamplesCount])
	return CreateSecondaryPeaksWith[T](peaks, left.primaryPeaks[:len(left.primaryPeaks)-contiguousEqualSamplesCount], left.originalPeaks)
}
...
// side, then it means that all the samples have the same value, and in that case
// we just add the 3 peaks, one for each sample.
func addContiguousPeaksToRightSide0[T Number](right SecondaryPeaks[T]) SecondaryPeaks[T] {
	if len(right.peaks) == 0 {
		...
		// Pass the 'right.originalPeaks' as the new 'primaryPeaks', because we just
		// synthetically created all the peak indices, and now we need to set the
		// 'primaryPeaks' to the original peak indices, i.e., the peak indices in the
		// primary array
		return CreateSecondaryPeaksWith[T](CreatePeaksWith[T](right.samples, right.createAllPeaks()), right.originalPeaks, right.originalPeaks)
	} else {
		return right
	}
}
...
func removeContiguousPeaksFromRightSide0[T Number](right SecondaryPeaks[T], sampleValue T) SecondaryPeaks[T] {
	lastPeak := 0
	contiguousEqualSamplesCount := 1
	...
	peaks := CreatePeaksWith[T](right.samples, right.peaks[contiguousEqualSamplesCount:])
	return CreateSecondaryPeaksWith[T](peaks, right.primaryPeaks[contiguousEqualSamplesCount:], right.originalPeaks)
}
